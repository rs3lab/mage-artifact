...
def scrape_xsbench_data(conn: sql.Connection, base_dir: Path) -> int:
    """Scrape XSBench timing from all log dirs under base_dir
    ...
    for log_dir in (x for x in base_dir.rglob('*') if x.is_dir()):  
        ...
        for log_file in log_dir.iterdir(): 
            ...
            # Extract runtime from XSBench stdout.
            avg_time = None
...
log_dir = Path(mind_root) / 'apps' / 'xsbench'
if not log_dir.is_dir():
    print('Error: $MIND_ROOT/apps/xsbench missing!', file=sys.stderr)
    sys.exit(1)

# Scrape Logs!
conn = sql.connect(':memory:')
scrape_xsbench_data(conn, log_dir)
...
plt.title('XSBench Page-Rank Throughput vs % Local Memory')
