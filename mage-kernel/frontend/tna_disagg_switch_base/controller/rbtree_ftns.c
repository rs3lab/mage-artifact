		...
		struct rb_node ***rb_link, struct rb_node **rb_parent)
{
	...
	while (*__rb_link) {
		...
		if (vma_tmp->vm_end > addr) {
			...
			{
				pr_rbtree("VMA: [0x%lx - 0x%lx] vs req [0x%lx - 0x%lx]\n",
						  vma_tmp->vm_start, vma_tmp->vm_end, addr, end);
				return -ENOMEM;
			}
...
// 	printf("prev[%p] addr[%lx] prev[%lx - %lx]\n", (void *)prev, addr, prev ? prev->vm_start : 0, prev ? prev->vm_end : 0);
...
static int ___mn_split_vma(struct mm_struct *mm, struct vm_area_struct *vma,
		const unsigned long addr, int new_below)
{
	struct vm_area_struct *new;
	int err;
	...
	pr_vma("Entered ___mn_split_vma\n");
	...
	*new = *vma;
	...
	if (data_original)
	{
		...
		pr_rbtree("* VMA [0x%lx, 0x%lx] addr [0x%lx] new [0x%lx, 0x%lx]\n",
				  vma->vm_start, vma->vm_end, addr,
				  new ? new->vm_start : 0, new ? new->vm_end : 0);
	}
...
{
	unsigned long long pow_of_two_size = get_pow_of_two_req_size(vma->vm_end - vma->vm_start);
	unsigned long long end_pow_2 = get_next_pow_of_two_addr(vma->vm_end, pow_of_two_size);
	unsigned long long start_pow_2 = end_pow_2 - pow_of_two_size;
	unsigned long long mid_addr;
	struct vm_area_struct *next;

	pr_rbtree("Decompose: [0x%lx - 0x%lx] | power_of_2 - lb: 0x%llx, rb: 0x%llx, size: 0x%llx\n",
			  vma->vm_start, vma->vm_end, start_pow_2, end_pow_2, pow_of_two_size);
	// Already aligned?
	...
	if (vma->vm_start < start_pow_2)	// TODO: this might cause a problem
										// if there exists previous vma before vma->vm_start
	{
		pow_of_two_size *= 2;
		start_pow_2 = end_pow_2 - pow_of_two_size;
	}
	mid_addr = (start_pow_2 + end_pow_2) / (unsigned long long)2;
