...
static struct hash_table mn_compute_hash;
...
int init_mn_man(void)
{
    ...
    ht_create(&mn_compute_hash, 1 << MN_PID_HASH_BIT);
...
struct task_struct *mn_get_task(u16 tgid)
{
    ...
    hnode = ht_get(&mn_compute_hash, hash_ftn_u8(tgid), tgid);
...
static struct unique_tgid_node *get_utgid(u16 tgid)
{
    struct node_tgid_hash *hnode = NULL;

    hnode = ht_get(&mn_compute_hash, hash_ftn_u8(tgid), tgid);
    if (hnode)
        printf("get_utgid: hnode found\n");
    if (hnode && hnode->utgid_node)
    {
...
{
    // sender_id = 0;
    struct unique_tgid_node *utgid = get_utgid(tgid);
    if (utgid)
    {
        utgid->local_ref++;
    }
    ...
    if (mm)
    {
        ...
        for (; cur; cur = cur->vm_next)
        {
            ...
            sprintf(str_data, "%s, %d, 0x%lx, 0x%lx, 0x%lx, %d, %d\n",
                    str_time, DISAGG_EXIT, cur->vm_start, cur->vm_end, cur->vm_flags, cur->vm_private_data ? 1 : 0, // actually accessed / allocated?
...
int mn_delete_task(u16 sender_id, u16 tgid, u16 pid_ns_id)
{
    ...
    printf("mn_delete_task: ht_get_list_head: sender_id: %u, tgid: %u\n", sender_id, tgid);

    tmp = ht_get_list_head(&mn_compute_hash, hash_ftn_u8(tgid));
    if (!tmp)
        printf("mn_delete_task: NOT FOUND!\n");

    while (tmp)
    {
        printf("mn_delete_task: sender_id: %u, tgid: %u\n", sender_id, tgid);
        ...
        if ((tmp->key == tgid) && hnode && (hnode->tgid == tgid) && hnode->utgid_node)
        {
            ...
            if (hnode->utgid_node->local_ref > 0)
            {
                ...
                res = hnode->utgid_node->local_ref;
                break;
            }
            ...
            // Hash
            ht_free_node(&mn_compute_hash, hash_ftn_u8(tgid), tmp);
            free(hnode);
...
{
    u32 utgid;
    u32 ns_tgid = generate_nstgid(tgid, pid_ns_id);
    ...
    hnode->node_id = sender_id; // NOT REALLY USED
    hnode->tgid = tgid;
    hnode->pid_ns_id = pid_ns_id;
    hnode->utgid_node = tgnode;

    printf("mn_insert_new_task_mm: new hnode inserted: sender_id %d, ns_tgid: %d\n", sender_id, ns_tgid);
    ht_put(&mn_compute_hash, hash_ftn_u8(tgid), tgid, hnode);
