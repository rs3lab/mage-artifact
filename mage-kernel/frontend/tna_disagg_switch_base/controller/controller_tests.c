...
static int test_insert_new_task()
{
    ...
    TEST_ASSERT_VALUES_EQ(mn_get_task(tgid), NULL);

    // get prev_task but no
    old_tsk = mn_get_task(prev_tgid);
    ...
    // 3) override existing
    TEST_ASSERT_VALUES_NEQ(mn_get_task(tgid), NULL);
...
// XXX(yash): We could use this to spoof add a memory node.
