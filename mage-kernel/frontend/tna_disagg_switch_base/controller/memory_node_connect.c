...
static void* zeroed_page = NULL;

void init_mn_virtual_address_man(void)
{
    zeroed_page = malloc(PAGE_SIZE);
...
// XXX(yash): Complete VMA mapping can be sent here...
...
//                    prev_addr, req_size);
    ...
    {
        fprintf(stderr, "Out of memory! Offset/Addr: 0x%lx, MN Size: 0x%lx\n", addr, mn_ptr->node_info->size);
        return 0;
    }
...
static unsigned long allocate_best_available_offset(int mn_node_idx, unsigned long size)
{
    ...
    if ((!offset))
    {
        return 0; // y: "zero offset" apparently is overloaded as an error
                  // condition.
    }

    zeros_mem_range(mn_node_idx, offset, size);
...
get_available_virtual_address_lock(unsigned long size, spinlock_t **mn_lock_ptr)
{
    ...
    *mn_lock_ptr = &mn_list[mn_node_idx]->alloc_lock;
    ...
    offset = get_best_available_offset(mn_node_idx, size);
    // Here, the "offset" should be memory offset in MIND addr space.
    // (will have to add memory node base offset to this to get the real DMA
    // addr though).

    if ((!offset))
        return 0;

    // The higher order bits encode the memory node number.
    // The lower order bits encode the memory offset. TODO which addr space?
    return ((MN_VA_PARTITION_BASE * mn_node_idx) | offset);
}

void add_new_addr_trans_rule(
        unsigned long addr, unsigned long size, // virtual region to map
        struct memory_node_mapping *mnmap, // mapping data
        unsigned int tgid, unsigned int permission, struct vm_area_struct *vma)
{
    // Size should be power of 2 (allocation granularity fix)
     ...
     */
    va_to_dma = mnmap->mn_stat->node_info->base_addr // base address of memory node
              + mnmap->base_addr                     // base address of mapped VMA
              - full_addr;                           // conversion from full_addr

    printf("Adding new Address Translation Rule: 0x%lx -> 0x%lx, size=%lu\n",
            addr, va_to_dma, size);

    bfrt_add_addr_except_trans(full_addr, prefix, mem_node_ip,
            va_to_dma, permission, !vma->is_test);
    free(mem_node_ip);
                            ...
                            unsigned int permission)
{
    ...
    printf("Modifying Address Translation Rule: 0x%lx -> 0x%lx, size=%lu\n",
            addr, va_to_dma, size);

    bfrt_modify_addr_except_trans(full_addr, prefix, mem_node_ip,
            va_to_dma, permission);
    free(mem_node_ip);
}
...
void del_addr_trans_rule(uint64_t addr, unsigned long size, struct vm_area_struct *vma)
{
    ...
    printf("Deleting Address Translation Rule: 0x%lx, size=%lu\n", addr, size);
                        ...
                        unsigned int permission, struct vm_area_struct *vma)
{
                             ...
                             (unsigned long)(mnmap->mn_stat->node_info->size)))
    {
        ...
        if (mnmap->base_addr)
             zeros_mem_range(mn_node_idx, mnmap->base_addr, size);
...
int send_meminit_request(struct dest_info *mnmap,
                         ...
                         unsigned long len)
{
    // y: the return value might be wrong here.
    int ret = sizeof(struct meminit_reply_struct);
    printf("Pretending to send DISAGG_MEM_INIT request to MN: ret=%d\n", ret);
    return ret;
}

int send_memcpy_request(struct dest_info *dest_mn,
                               unsigned long old_off, unsigned long new_off,
                               unsigned long copy_len)
{
    printf("Pretending to send DISAGG_MEM_COPY request to MN (0x%lx->0x%lx, size=%lu)\n",
            old_off, new_off, copy_len);
    return 0;
}
