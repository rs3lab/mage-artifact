...
get_unmapped_area:
#ifdef VM_ALLOC_TOPDOWN
    info.flags = VM_UNMAPPED_AREA_TOPDOWN;
#else
    info.flags = 0;
#endif
    info.length = len;
    info.low_limit = MN_VA_MIN_ADDR;
                               ...
                               const unsigned long va_start, const unsigned long va_end)
{
    ...
    info.flags = 0;
    info.length = len;
    info.low_limit = va_start;
    info.high_limit = va_end;
    info.align_mask = 0;
    info.align_offset = 0;
    info.kernel_memory = 1;
    addr = mn_vm_unmapped_area(tsk, &info);
    return addr;
}

unsigned long
mn_arch_kern_check_and_get_va(struct task_struct *tsk, unsigned long addr,
