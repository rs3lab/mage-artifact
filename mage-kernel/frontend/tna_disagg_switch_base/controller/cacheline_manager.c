...
// Counters
static atomic_int free_dir_cnt;
static atomic_int dir_print_cnt;
...
static void cacheline_freelist_init(void)
{
    ...
    pr_cache("cacheline_freelist_init: ht_create\n");
...
static void cacheline_freelist_clear(void)
{
    ...
    pr_cache("cacheline_freelist_clear: ht_free(&cache_directory)\n");
...
inline void *create_new_cache_dir(u64 fva, u16 state, int nid, uint16_t dir_size)
{
    ...
    pr_cache("Entry for FVA: 0x%lx, hash: 0x%lx, key: 0x%lx\n",
    ...
    if (!hash_node)
    {
        ...
        pr_cache("fva[%lx] is aligned to fva[%lx]\n", fva, aligned_fva);
        uint32_t inv_cnt = 0;
        ...
        {
            // sharer = 1 << (nid - 1);
            printf("ERROR: nid must not be used for creating cache\n");
            printf("Terminating the whole process...\n");
            exit(-1);
        }
...
// y: It doesn't seem to actually do anything other than print things.
//    This used to handle UDP requests -- but I deleted that thread.
...
u64 get_left_uncached_addr(u16 tgid, u64 addr, u64 begin_addr) {
    unsigned long fva = get_full_virtual_address(tgid, addr);
    unsigned long aligned_fva = get_aligned_fva(fva, REGION_SIZE_BASE + REGION_SIZE_2MB);
    aligned_fva = max(aligned_fva, get_full_virtual_address(tgid, begin_addr));
    hlist *hash_node;
	while (aligned_fva < fva) {
		pthread_spin_lock(&cacheline_hashtb_lock);
    	hash_node = cacheline_get_node_no_lock(aligned_fva);
		if (hash_node) {
            pthread_spin_unlock(&cacheline_hashtb_lock);
			break;
		} else {
			aligned_fva += CACHELINE_MIN_SIZE;
		}
		pthread_spin_unlock(&cacheline_hashtb_lock);
	}
    return aligned_fva & (~MN_VA_PID_BIT_MASK);
}

u64 get_right_uncached_addr(u16 tgid, u64 addr) {
    unsigned long fva = get_full_virtual_address(tgid, addr);
    unsigned long aligned_fva = get_aligned_fva(fva, REGION_SIZE_BASE + REGION_SIZE_2MB);
    hlist *hash_node;
	while (aligned_fva < fva) {
    	...
    	hash_node = cacheline_get_node_no_lock(aligned_fva);
		if (hash_node) {
			aligned_fva += (1 << (((struct cacheline_dir *)(hash_node->value))->dir_size + REGION_SIZE_BASE));
		} else {
			aligned_fva += CACHELINE_MIN_SIZE;
		}
		pthread_spin_unlock(&cacheline_hashtb_lock);
