...
//constexpr static uint64_t kTotalAccesses = 23177824;
//constexpr static uint64_t kTotalAccesses = 23177824;
constexpr static uint64_t kTotalAccesses = 23177824;
...
size_t getMajor(){
  ...
  unsigned long major;
  char buff[20];
  if (fscanf(fp, "%d %s %c %d %d %d %d %d %lu %lu %lu %lu ...", &unused, &buff, &unused, &unused, &unused, &unused, &unused, &unused, &unused, &unused, &unused, &major) != 12) {
	  perror("");
	  exit(1);
  }
...
size_t getMinor(){
  ...
  unsigned long minor;
  char buff[20];
  if (fscanf(fp, "%d %s %c %d %d %d %d %d %lu %lu ...", &unused, &buff, &unused, &unused, &unused, &unused, &unused, &unused, &unused, &minor) != 10) {
	  perror("");
	  exit(1);
  }
...
bool random_access, bool exclusive, double zipfian){
    ...
    if (latency_test){
        ...
        uint64_t p99_position = count / kSampleGap / 500;
