...
{
    // NOTE: 2 CPUs are hard coded for phase changing experiments
    //int MAP_CPUS = 2;
    int ncpus = (type == MERGE) ? mr_state.merge_ncpus :
	mr_state.mr_fixed.nr_cpus;
    // NOTE: Hardcoded CPUs for phase changing experiments
    // ncpus = (type == MAP) ? MAP_CPUS: ncpus; 
    prof_phase_stat st;
...
mr_run_scheduler(mr_param_t * param)
{
    ...
    // Note: This code is used to get the stats for the map phase from Mage/Dilos
    /*
    typedef struct {
        uint64_t fpos;
        uint64_t flen;
    ...
    madvise((void*)(((wr_data_t *)param->split_arg)->fdata), 0, 0x103);
    fflush(stdout);
    */
...
#define SEP "\t"
    ...
    printf("Real:\t%" PRIu64 SEP, total_real_time);
    printf("\nNumber of Tasks\n\t");
    if (the_app.atype == atype_maponly) {
