...
class FarMemTest {
public:
  // Scales benchmark WSS.
  // 0 => size x. 2 => x/4. 7 => x/128
  constexpr static uint32_t kScaleShift = 2;

  constexpr static uint32_t kNumMutatorThreads = 48;
  constexpr static uint32_t kThreadSleepTime = 20;
private:
  // Hashtable.
  constexpr static uint32_t kKeyLen = 12;
  constexpr static uint32_t kValueLen = 4;
  constexpr static uint32_t kLocalHashTableNumEntriesShift = 28 - kScaleShift;
  constexpr static uint32_t kNumKVPairs = 1 << (27 - kScaleShift);
  constexpr static uint64_t kHTDataSize = 2ULL * (kNumKVPairs) * (3 + kKeyLen + kValueLen); // KVDataHeader = 3

  // Array.
  constexpr static uint32_t kNumArrayEntries = (2 << 20) >> kScaleShift; // 2 M entries.
