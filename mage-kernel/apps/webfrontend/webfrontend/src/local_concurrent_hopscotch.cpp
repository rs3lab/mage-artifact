...
void LocalGenericConcurrentHopscotch::get(uint8_t key_len, const uint8_t *key,
  ...
  auto get_once = [&]<bool Lock>() -> bool {
    if constexpr (Lock) {
      while (unlikely(!bucket->spin_.trylock())) {
        sched_yield();
      }
    }
...
remove:
  bucket->spin_.lock();
  auto spin_guard = helpers::finally([&]() { bucket->spin_.unlock(); });
  if (likely(entry->ptr)) {
...
bool LocalGenericConcurrentHopscotch::put(uint8_t key_len, const uint8_t *key,
  ...
  auto bucket_lock_guard = helpers::finally([&]() { bucket->spin_.unlock(); });
  ...
  while ((distance_to_orig_bucket = bucket_idx - orig_bucket_idx) >=
    ...
    for (distance = kNeighborhood - 1; distance > 0; distance--) {
      ...
      while (unlikely(!anchor_entry->spin_.trylock())) {
        sched_yield();
      }
      auto lock_guard =
          helpers::finally([&]() { anchor_entry->spin_.unlock(); });
