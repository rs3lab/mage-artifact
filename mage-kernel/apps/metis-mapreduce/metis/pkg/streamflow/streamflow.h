...
struct superpage {
    ...
    lock_t *lock;
    /* points to the lock that belongs to the thread
     * that allocated it. */
...
struct pageblock {
    struct superpage *sph;
    /* points to the superpage header; needs to
    ...
    union {
	struct {
	    /* indicates which thread owns the pageblock */
	    volatile unsigned int owning_thread;
	    /* objects that need to be garbage collected in this pageblock */
	    volatile struct queue_node garbage_head;
	};
