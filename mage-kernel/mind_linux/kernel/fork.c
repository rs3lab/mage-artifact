...
#ifndef CONFIG_COMPUTE_NODE
static int disagg_fork(unsigned long clone_flags, struct task_struct *tsk)
{
    ;
}
#else
...
/*
 * initialize remote memory tasks
 */
#define MAX_REMOTE_TARGET_USER 4
static uid_t _remote_target_uid_list[MAX_REMOTE_TARGET_USER];

void __init remote_mem_task_init(void)
{
	int i = 0;
	for (i = 0; i < MAX_REMOTE_TARGET_USER; i++)
	{
		_remote_target_uid_list[i] = -1;
	}

	//first user
	_remote_target_uid_list[0] = 1001;
}

bool is_remote_task(uid_t uid){
	int i = 0;
	bool is_remote = false;
	for (i = 0; i < MAX_REMOTE_TARGET_USER; i++)
	{
		if (_remote_target_uid_list[i] == (int)uid)
		{
			is_remote = true;
			break;
		}
...
void mm_release(struct task_struct *tsk, struct mm_struct *mm)
{
	...
	if (unlikely(tsk->robust_list)) {
		// exit_robust_list(tsk);	// TODO: postpone disagg_exit and re-enable this
...
static int copy_sighand(unsigned long clone_flags, struct task_struct *tsk)
{
	...
	if (clone_flags & CLONE_SIGHAND) { // don't support CLONE_SIGHAND for now.
					...
					int node)
{
	...
	if (current->is_remote)
		pr_syscall("copy_process_started\n");
	...
	if (clone_flags & CLONE_THREAD) {
		if ((clone_flags & (CLONE_NEWUSER | CLONE_NEWPID)) ||
			...
			pr_syscall("copy_process: new process will be in a different pid or user namespace\n");
			return ERR_PTR(-EINVAL);
		}
	}
	...
	// set up remote based on user id in cred
	if (current->is_remote && current->is_mind_binary)
	{
		p->is_remote = p->is_mind_binary = 1;
	}
	else
	{
		// printk(KERN_DEFAULT "copy_process: is_remote_task(p->cred->uid.val) \n");
	...
	p->pid_ns_id = current->nsproxy->pid_ns_for_children->pid_ns_id;
	if (retval) {
	...
	*/
	...
	if (p->is_remote){
		...
		// (sleep-while-atomic). So I commented out the `alloc_lock`
		// lines, with an imperfect understanding of what the locks
		// protected. Beware.
		//
	      ...
	      unsigned long tls)
{
	...
	if (!IS_ERR(p)) {
		...
		if (p->is_remote) {
			// finish mm initialization by setting up ASID
			p->mm->is_remote_mm = true;
			p->mm->remote_asid = find_next_avail_disagg_asid(p->mm);
			pr_syscall("MIND - FORK | New tgid[%u] for ASID[%u]\n", p->tgid, p->mm->remote_asid);
			// pr_syscall("do_fork: final number of mm users: %d\n", atomic_read(&p->mm->mm_users));

			// Assign the task a unique RDMA QP.
			BUG_ON(mind_rdma_get_fhqp_handle_fn == NULL);
			p->qp_handle = mind_rdma_get_fhqp_handle_fn();
			if (current->qp_handle == NULL)
				 pr_err("Fork: ERROR: couldn't assign FHQP handle\n");
			else
				 pr_rdma("Fork: assigned FHQP handle %d\n", current->qp_handle);
		}
#endif
