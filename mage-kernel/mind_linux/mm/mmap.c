...
static void __insert_vm_struct(struct mm_struct *mm, struct vm_area_struct *vma)
{
	...
	if (find_vma_links(mm, vma->vm_start, vma->vm_end,
		...
		{
			printk(KERN_ERR "VMA EXIST: 0x%lx - 0x%lx\n",
				   vma->vm_start, vma->vm_end);
			BUG();
		}
	__vma_link(mm, vma, prev, rb_link, rb_parent);
...
/*
 * Filter for target app or user
 */
void print_remote_alloc(unsigned long len)
{
#ifdef CONFIG_DISAGGREGATION_DEBUG
	uid_t _uid = get_current_user()->uid.val;
	pid_t _pid = current->pid;
	char* _th_name = current->comm;

	if (_th_name)
	{
		printk(KERN_DEFAULT "mmap - uid: %d, pid: %d, size: %lu, proc: %s\n",
				(int)_uid, (int)_pid, len, _th_name);
	}
#endif
}
...
#ifdef CONFIG_COMPUTE_NODE
	if (current->is_remote) {
		pr_syscall(KERN_DEFAULT "MMAP (before lock) - addr: %lx, len: %lu, cpu: %d\n", addr, len, smp_processor_id());
	}
#endif
...
static inline void print_out_debug_mmap(const char *err_str, int line)
{
	#ifdef CONFIG_COMPUTE_NODE
	printk(KERN_DEFAULT "%s:L%d - error: %s\n", __func__, line, err_str);
	#endif
}
		...
		struct list_head *uf)
{
	...
	if (!may_expand_vm(mm, vm_flags, len >> PAGE_SHIFT)) {
		...
		{
			printk(KERN_DEFAULT "npages: %ld (nr_page: %ld) <-> %ld\n",
				   mm->total_vm + (len >> PAGE_SHIFT) - nr_pages,
				   nr_pages, rlimit(RLIMIT_AS) >> PAGE_SHIFT);
			print_out_debug_mmap("ERROR_expand_vm", __LINE__);
			DEBUG_print_vma(mm);
			return -ENOMEM;
		}
	}
	...
	while (find_vma_links(mm, addr, addr + len, &prev, &rb_link,
		...
		{
			print_out_debug_mmap("ERROR_do_munmap", __LINE__);
			return -ENOMEM;
		}
	}
	 ...
	 */
	if (accountable_mapping(file, vm_flags)) {	//check flag (ignore hugepage file)
		charged = len >> PAGE_SHIFT;
		if (security_vm_enough_memory_mm(mm, charged))	//check memory (mm, # of pages)
		{
			print_out_debug_mmap("ERROR_accountable_map", __LINE__);
			return -ENOMEM;
		}
		vm_flags |= VM_ACCOUNT;
	}
...
out:
	...
	/* mm/mmap.c, Update vma->vm_page_prot to reflect vma->vm_flags. */
...
unsigned long mmap_dummy_region(struct mm_struct *mm, unsigned long addr, unsigned long len, vm_flags_t vm_flags)
{
	struct vm_area_struct *vma, *prev;
	int error;
	struct rb_node **rb_link, *rb_parent;
	 ...
	 * Private writable mapping: check memory availability
	 */
	if (accountable_mapping(NULL, vm_flags))
	{ //check flag (ignore hugepage file)
		charged = len >> PAGE_SHIFT;
		if (security_vm_enough_memory_mm(mm, charged)) //check memory (mm, # of pages)
			return -ENOMEM;
		vm_flags |= VM_ACCOUNT;
...
#ifdef CONFIG_COMPUTE_NODE
static void print_err_in_stack_expand(int error, char* st)
{
	if (current->is_remote){
		printk(KERN_DEFAULT "*Stack expand error (%d) : %s\n", error, st);
	}
}
#else
static void print_err_in_stack_expand(int error, char* st){;}
				   ...
				   unsigned long address)
{
	...
	if (error){
		print_err_in_stack_expand(error, "security_mmap_addr");
		return error;
	}
	...
	if (prev && !(prev->vm_flags & VM_GROWSDOWN) &&
		...
		{
			print_err_in_stack_expand(-ENOMEM, "stack_guard_gap");
			printk(KERN_DEFAULT "- addr: 0x%lx, prev_addr: 0x%lx - 0x%lx\n", 
				address, prev->vm_start, prev->vm_end);
			return -ENOMEM;
		}
	}
	...
	if (unlikely(anon_vma_prepare(vma)))
	{
		print_err_in_stack_expand(-ENOMEM, "anon_vma_prepare");
		return -ENOMEM;
	}
	...
	if (address < vma->vm_start) {
		...
		if (grow <= vma->vm_pgoff) {
			...
			if (!error) {
				...
				print_err_in_stack_expand(error, "acct_stack_growth");
			}
		}
		if (error)
		{
#ifdef CONFIG_COMPUTE_NODE
			if (current->is_remote)
				printk(KERN_DEFAULT "grow: 0x%lx, size: 0x%lx, pg_off: 0x%lx\n", 
						grow, size, vma->vm_pgoff); 
#endif
		}
	}
	anon_vma_unlock_write(vma->anon_vma);
...
void unmap_pages_vma(struct mm_struct *mm, struct vm_area_struct *vma)
{
	struct mmu_gather tlb;
	lru_add_drain();
	tlb_gather_mmu(&tlb, mm, vma->vm_start, vma->vm_end);
	update_hiwater_rss(mm);
	// unmap_vmas(&tlb, vma, vma->vm_start, vma->vm_end);
...
static int do_brk_flags(unsigned long addr, unsigned long request, unsigned long flags, struct list_head *uf)
{
	...
	if (flags != BRK_SYSCALL_FLAG_DEF)
	{
		...
		flags = 0;
		flags |= VM_DATA_DEFAULT_FLAGS | VM_ACCOUNT | mm->def_flags;
		flags &= ~(VM_READ | VM_WRITE);		// no write permission
	}
...
#ifdef CONFIG_COMPUTE_NODE
	if (current->is_remote && vma)
	{
		pr_syscall(KERN_DEFAULT "VM_BRK (local) - addr: 0x%lx, len: %lu, vma: 0x%lx - 0x%lx\n",
				   addr, len, vma->vm_start, vma->vm_end);
	}
#endif	
