					...
					unsigned long check_flags)
{
	...
	{
		// if (atomic_read(&page->_mapcount) == 0 && page_ref_count(page) == 0)
		// {
		// 	;	// pages managed by cnthread
		// }
		// else
		{
			return false;
...
static void free_pages_check_bad(struct page *page)
{
	...
	if (unlikely(atomic_read(&page->_mapcount) != -1))
	{
		...
		printk(KERN_ERR "mapcount: %d, refcount: %d\n",
		...
		unsigned long start_pfn, enum memmap_context context)
{
	...
	if (highest_memmap_pfn < end_pfn - 1) {
		...
		pr_syscall("memmap_init_zone: highest_memmap_pfn initialized to: %ld\n", highest_memmap_pfn);
	}
