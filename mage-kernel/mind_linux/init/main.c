...
extern void disagg_cn_tlb_init(void);
...
#ifdef CONFIG_COMPUTE_NODE
	/* network-related variables initialization for disaggregated system */
	disagg_network_init();
#endif
...
#ifdef CONFIG_COMPUTE_NODE
	printk(KERN_WARNING "shoop: before disagg cn init\n");
	init_pps(); // init profiling points for disaggregated system.

	// TODO suboptimal allocation strategy when fhthreads != app threads.
	BUILD_BUG_ON(NUM_FHTHREADS + NUM_CNTHREADS > NUM_ASSIGNABLE_CPUS);
	init_disagg_core_pinning(NUM_FHTHREADS);
