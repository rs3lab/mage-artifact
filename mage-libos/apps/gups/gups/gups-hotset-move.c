...
#define PAGE_NUM            3
#define PAGES               2048
#define PHASE_CHANGE_TIME 10
#define END_TIME  15
#define AI 10 // Arithmetic intensity
...
double elapsed(struct timeval* start, struct timeval* end) {
    double start_sec = start->tv_sec + start->tv_usec / 1e6;
    double end_sec = end->tv_sec + end->tv_usec / 1e6;
...
static void *timing_thread()
{
  ...
  while(!all_done){
    tic++;
    if (tic >= PHASE_CHANGE_TIME && !printed1) {
      move_hotset1 = true;
      fprintf(stderr, "moved hotset1\n");
      printed1 = true;
    } else if (tic >= END_TIME){
      stop = true;
    }
    sleep(1);
  }
...
static void *touch_mem(void *arguments)
{
  ...
  for (i = 0; i < args->size; i++){
    memset(args->field + i * elt_size, 
      (i + args->tid) % 256, elt_size);
  }
  return 0;
}

// Before move we touch 80% after move we touch 20%
static void *do_gups(void *arguments)
{
  ...
  printf("Thread %d region: %p - %p\thot set: %p - %p\n", 
    args->tid, args->field, 
    args->field + (args->size * elt_size), 
    args->field + args->hot_start, 
    args->field + args->hot_start + (args->hotsize * elt_size));   
  ...
  for (i = 0; i < args->iters; i++) {
    ...
    if (true) {
      lfsr = lfsr_fast(lfsr);
      index1 = move_hotset1 ? args->hot_start + args->hotsize * 8 / 10 + (lfsr % (args->hotsize * 2 / 10)) : 
        args->hot_start + (lfsr % (args->hotsize * 8 / 10));
      if (elt_size == 8) {
        ...
        //memcpy(data, args->field + index1 * elt_size, elt_size);
        //memset(data, data[0] + i, elt_size);
        //memcpy(args->field + index1 * elt_size, data, elt_size);
	//uint64_t *ptr = (uint64_t*) (args->field + index1 * elt_size);
	//*ptr = *ptr + i;
        memcpy(data, args->field + index1 * elt_size, 8);
        memset(data, data[0] + i, 8);
        memcpy(args->field + index1 * elt_size, data, 8);
      }
    }
    else {
      lfsr = lfsr_fast(lfsr);
      // index2 = lfsr % (args->size);
      index2 = move_hotset1 ? args->size * 8 / 10 + (lfsr % (args->size * 2 / 10)) : 
        lfsr % (args->size * 8 / 10);
      if (elt_size == 8) {
        ...
        //memcpy(data, args->field + index2 * elt_size, elt_size);
        //memset(data, data[0] + i, elt_size);
        //memcpy(args->field + index2 * elt_size, data, elt_size);
        memcpy(data, args->field + index2 * elt_size, 8);
        memset(data, data[0] + i, 8);
        memcpy(args->field + index2 * elt_size, data, 8);
      }
    }
...
int main(int argc, char **argv)
{
  ...
  fprintf(stderr, "%lu updates per thread (%ld threads)\n", updates, threads);
  fprintf(stderr, "field of 2^%lu (%lu) bytes\n", expt, size);
  fprintf(stderr, "%ld byte element size (%ld elements total)\n", elt_size, size / elt_size);

  p = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_DDC, -1, 0);
  if (p == MAP_FAILED) {
    perror("mmap");
    assert(0);
  }
  ...
  printf("Touch\n");
  for (i = 0; i < threads; i++) {
    int r = pthread_create(&t[i], NULL, touch_mem, (void*)ga[i]);
    assert(r == 0);
  }
