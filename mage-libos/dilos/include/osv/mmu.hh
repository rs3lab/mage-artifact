...
extern percpu<uint64_t> tlb_local_flush;
extern percpu<uint64_t> tlb_check_cpus;
extern percpu<uint64_t> tlb_send_ipi;
extern percpu<uint64_t> tlb_wait_ipi;
extern percpu<uint64_t> _percpu_vmfault_cycles;
extern percpu<uint64_t> _percpu_rdma_cycles;
extern percpu<uint64_t> _percpu_rdma_sched_cycles;
...
}cachelines_t CACHELINE_ALIGNED;

// Magic number here !!
extern cachelines_t tlb_flush_cachelines[sched::max_cpus];
...
constexpr inline unsigned pt_index(ulong virt, unsigned level)
{
    return (virt >> (page_size_shift + level * pte_per_page_shift)) & (pte_per_page - 1);
}
...
void virt_to_phys(void* vaddr, size_t len, OutputFunc out)
{
    if (CONF_debug_memory && vaddr >= debug_base) {
        while (len) {
            auto next = std::min(align_down((char*)vaddr + page_size, page_size), (char*)vaddr + len);
