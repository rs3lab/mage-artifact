...
extern std::string opt_sync_reclaim;
extern std::string opt_async_reclaim;
extern std::string opt_tlb_flush_mode;
extern unsigned opt_max_reclaim_threads;
extern bool opt_no_global_l2;

extern percpu<size_t> maj_flts;
extern percpu<size_t> min_flts;
...
extern size_t watermark_lo;
extern size_t watermark_hi;
...
// This class is temporarily unusable because the free_page_ranges_lock has been 
// destroyed.
...
public:
    reclaimer ();
    void wake();
    void concurrent_wake_all(); // Wake all the concurrent _reclaim_threads
    void concurrent_wake_first(); // Wake the first concurrent _reclaim_threads
    // void wait_for_memory(size_t mem);
    // void wait_for_memory_conditional(size_t mem); // Wait if _oom_blocked has waiter
    // void wait_for_minimum_memory();
    void sync_reclaim_for_minimum_memory();
    size_t do_sync_reclaim(size_t target);
    static pressure pressure_level();

    void add_threads(size_t n_more_threads);

    friend class shrinker;
    friend class reclaimer_waiters;
private:
    void _do_concurrent_reclaim(size_t id);
    ...
    std::vector<std::unique_ptr<sched::thread>> _reclaim_threads;

    std::atomic<size_t> _additional = {0};
    std::vector<shrinker *> _shrinkers;
    mutex _shrinkers_mutex;
    unsigned int _active_shrinkers = 0;
    bool _can_shrink();

    // Not acquire the free_page_ranges_lock
    ssize_t bytes_until_normal(pressure curr);
    ssize_t bytes_until_normal() { return bytes_until_normal(pressure_level()); }

    ssize_t bytes_until_relaxed(pressure curr);
    ssize_t bytes_until_relaxed() { return bytes_until_relaxed(pressure_level()); }
    const size_t reclaimer_batch_size = 1024 * page_size;
};
...
namespace stats {
    ...
    // NOTE: Only count those from remote
    size_t maj_flt();
    size_t min_flt();
}
