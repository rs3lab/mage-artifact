...
enum {
    tlb_from_cooperative = 1ul << 63,
};

/* TODO: Check what will happen for concurrent flush where there is 
 * another flush (send) happens before the ack */
/* Send TLB flush without ack */
void flush_tlb_all_send_pipelined(unsigned current_cpu);
/* Ack TLB flush */
void flush_tlb_all_ack_pipelined(unsigned current_cpu);
...
/* flush tlb on behalf of the application thread.*/
/* Only implemented for x86 now */
/* later we can think of batching the TLB flush from the reclaim side*/
void flush_tlb_on_behalf(unsigned request_cpu);

/* flush tlb cooperatively. Only enqueue and entry without IPI */
/* only implemented for x86 now */
void flush_tlb_cooperative();

/* Check whether the cooperative tlb has finished or not */
/* only implemented for x86 now */
int flush_tlb_cooperative_check();

/* Update the queue after local tlb flush */
/* only implemented for x86 now */
void flush_tlb_update_queue(unsigned current_cpu);

/* Check whether tlb flush is needed or not */
int flush_tlb_need_flush_check(unsigned current_cpu);

/* Check whether tlb flush is needed and flush if needed */
