...
static std::string procfs_stats()
{
    ...
    // NOTE: cmin_flt and cmaj_flt are faults from wait-for children which does not 
    // exist on OSv
    // NOTE: Now min_flt and maj_flt only counts those from remote
    int min_flt = 0, cmin_flt = 0, maj_flt = 0, cmaj_flt = 0;
    unsigned long stime = 0, cutime = 0, cstime = 0;

    using namespace std::chrono;
    unsigned long utime = duration_cast<nanoseconds>(sched::osv_run_stats()).count();
    char state = 'R';
    ...
    maj_flt = memory::stats::maj_flt();
    min_flt = memory::stats::min_flt(); 

    // NOTE: Since our system is tailor for remote memory we use stime to record the 
    // fault cycles 
    // NOTE: for the sake of time use cstime for rdma all.
    // NOTE: for the sake of time we use cutime for rdma scheduling.
    for (auto c: sched::cpus){
        stime += *(_percpu_vmfault_cycles.for_cpu(c));
        cstime += *(_percpu_rdma_cycles.for_cpu(c));
        cutime += *(_percpu_rdma_sched_cycles.for_cpu(c));
    }
