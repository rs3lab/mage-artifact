...
// Mage additional configs
std::string opt_sync_reclaim;
std::string opt_async_reclaim;
std::string opt_lru_mode;
std::string opt_tlb_flush_mode;
std::string opt_rdma_mode;
bool opt_no_scan_flush = false;
bool opt_no_global_l2 = false;
const unsigned opt_init_reclaim_threads = 1;
unsigned opt_max_reclaim_threads = opt_init_reclaim_threads;
...
static void usage()
{
    ...
    std::cout << "Mage additional options:\n";
    // Different modes for synchronous eviction
    // no: no synchronous reclaim. Poll waiting for physical page.
    // normal: perform synchronous reclamation the same as asynchronous reclamation
    // response: perform synchronous reclamation optimized for response time
    std::cout << "  --sync-reclaim=arg    synchronous reclamation mode, options: no, normal, response\n";
    std::cout << "  --async-reclaim=arg   asynchronous reclamation mode, options: normal, pipelined\n";
    // Different modes for LRU lists
    std::cout << "  --lru-mode=arg        lru mode: lru, sharded_lru, fifo, sharded_fifo\n";
    // Flush mode configurations
    std::cout << "  --tlb-flush-mode=arg  tlb flush mode: patr, batch\n";
    // Scan flush or unmap flush
    std::cout << "  --disable-scan-flush  Whether flush tlb or not after scanning\n";
    // Use global L2 or not
    std::cout << "  --disable-global-l2   Disable global l2 page allocator\n";
    std::cout << "  --max-reclaim-threads The number of background reclaim threads\n";
    // RDMA signaling each one or controlled manually
    std::cout << "  --rdma-mode=arg       Enable selective (batch) or individual signal\n";
}
...
static void parse_options(int loader_argc, char** loader_argv)
{
    ...
    // Mage additional configurations for better sensitivity analysis
    if (options::option_value_exists(options_values, "sync-reclaim")){
        opt_sync_reclaim = options::extract_option_value(options_values, "sync-reclaim");
        if (opt_sync_reclaim != "no" && opt_sync_reclaim != "normal" && opt_sync_reclaim != "response"){
            debug("Unrecognizable sync_reclaim mode: %s\n", opt_sync_reclaim);
            debug("Change sync_reclaim mode to normal\n");
            opt_sync_reclaim = "normal";
        }
    } else {
        opt_sync_reclaim = "normal";
    }

    if (options::option_value_exists(options_values, "async-reclaim")){
        opt_async_reclaim = options::extract_option_value(options_values, "async-reclaim");
        if (opt_async_reclaim != "pipelined" && opt_async_reclaim != "normal"){
            debug("Unrecognizable async_reclaim mode: %s\n", opt_async_reclaim);
            debug("Change async_reclaim mode to normal\n");
            opt_async_reclaim = "normal";
        }
    } else {
        opt_async_reclaim = "normal";
    }

    if (options::option_value_exists(options_values, "rdma-mode")){
        opt_rdma_mode = options::extract_option_value(options_values, "rdma-mode");
        if (opt_async_reclaim != "pipelined" && opt_rdma_mode == "selective"){
            debug("Selective signal must be set with pipelined async reclamation");
            debug("Change rdma_mode to individual\n");
            opt_rdma_mode = "individual";
        }
    } else {
        opt_rdma_mode = "individual";
    }

    if (options::option_value_exists(options_values, "lru-mode")){
        opt_lru_mode = options::extract_option_value(options_values, "lru-mode");
    } else {
        opt_lru_mode = "static_lru";
    }

    if (options::option_value_exists(options_values, "tlb-flush-mode")){
        opt_tlb_flush_mode = options::extract_option_value(options_values, "tlb-flush-mode");
    } else {
        opt_tlb_flush_mode = "patr";
    }

    if (options::option_value_exists(options_values, "max-reclaim-threads")){
        opt_max_reclaim_threads = options::extract_option_int_value(options_values, 
            "max-reclaim-threads", handle_parse_error);
    } else {
        opt_max_reclaim_threads = 2;
    }

    if (extract_option_flag(options_values, "disable-scan-flush")){
        opt_no_scan_flush = true;
    }
    
    if (extract_option_flag(options_values, "disable-global-l2")){
        opt_no_global_l2 = true;
    }
...
void main_cont(int loader_argc, char** loader_argv)
{
    ...
    memory::add_additional_reclaim_threads(
        opt_max_reclaim_threads - opt_init_reclaim_threads);
