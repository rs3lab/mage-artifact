...
TRACEPOINT(trace_memory_call_wait, "call wait from %d", size_t);
TRACEPOINT(trace_memory_wakeup_waiter, "range size=%d, request size=%d", size_t, size_t);
TRACEPOINT(trace_memory_loop, "range size=%d, done", size_t);
TRACEPOINT(trace_memory_patch, "total size=%d, request size=%d", size_t, size_t);
TRACEPOINT(trace_pr_alloc, "physical range allocator alloc size=%d, free size=%d", size_t, size_t);
TRACEPOINT(trace_pr_pointer, "physical range allocator, page pointer=%p", void *);
TRACEPOINT(trace_pr_case, "physical range allocator, case=%d", size_t);
TRACEPOINT(trace_l2_unfill, "l2 unfill n_batches=%d thres=%d", size_t, size_t);
TRACEPOINT(trace_l2_free_page_batch, "l2 free_page_batch n_batches=%d lo_wm=%d hi_wm=%d", size_t, size_t, size_t);
TRACEPOINT(trace_l2_alloc_page_batch, "l2 alloc_page_batch n_batches=%d lo_wm=%d hi_wm=%d", size_t, size_t, size_t);
TRACEPOINT(trace_l2_concurrent_wake_all, "l2 wake all concurrent");
TRACEPOINT(trace_l2_concurrent_wake_one, "l2 wake one concurrent");
TRACEPOINT(trace_l2_sync_reclaim_working, "l2 sync reclaim, target=%d, freed=%d", size_t, size_t);
TRACEPOINT(trace_l2_refill, "l2 refill enter");
TRACEPOINT(trace_l2_refill_ret, "l2 refill exit");
TRACEPOINT(trace_concurrent_working, "concurrent reclaim working, target=%d, freed=%d, pressure_level=%d", size_t, size_t, size_t);
TRACEPOINT(trace_l1_unfill, "l1 unfill n_pages=%d thres=%d", size_t, size_t);
TRACEPOINT(trace_l1_free_page_local, "l1 free_page_local=%p n_pages=%d thres=%d", void *, size_t, size_t);
TRACEPOINT(trace_l1_alloc_page_local, "l1 alloc_page_local=%p n_pages=%d thres=%d", void *, size_t, size_t);
TRACEPOINT(trace_do_sync_reclaim_small, "");
TRACEPOINT(trace_do_sync_reclaim_small_ret, "");
TRACEPOINT(trace_do_sync_reclaim_medium, "");
TRACEPOINT(trace_do_sync_reclaim_medium_ret, "");
TRACEPOINT(trace_do_sync_reclaim_large, "");
TRACEPOINT(trace_do_sync_reclaim_large_ret, "");
TRACEPOINT(trace_l2_refill_wait, "%d", size_t);
TRACEPOINT(trace_l2_refill_all, "%d", size_t);
TRACEPOINT(trace_l2_refill_locking, "%d", size_t);
TRACEPOINT(trace_l2_refill_wait_for_page, "");
...
static inline void tracker_forget_n(int n_pages, void *addr_arr[])
{
    if (__builtin_expect(tracker_enabled, false)) {
        for (int i = 0; i < n_pages; i++)
            tracker.forget(addr_arr[i]);
    }
}
...
spinlock_t free_page_ranges_lock;
...
size_t watermark_lo(0);
size_t watermark_hi(0);
...
void wake_reclaimer()
{
    ...
    reclaimer_thread.concurrent_wake_all();
}
...
{
    // TODO: we can safely add dynamic watermark here
    total_memory.fetch_add(mem);
    watermark_lo = stats::total() * 10 / 100;
    watermark_hi = stats::total() * 60 / 100;
}
...
namespace stats {
    ...
    // This can be inprecise but we don't care here
    size_t maj_flt() {
        size_t sum = 0;
        for (auto c : sched::cpus) {
            sum  += *(maj_flts.for_cpu(c));
        }
        return sum;
    }
...
void reclaimer::concurrent_wake_all(){
    for (auto &thread: _reclaim_threads){
        thread->wake();
    }    
}

void reclaimer::concurrent_wake_first(){
    if (!_reclaim_threads.empty()){
        _reclaim_threads[0] -> wake();
    }
}

pressure reclaimer::pressure_level()
{
    size_t free_memory = stats::free();
    if ( free_memory < watermark_lo) {
        return pressure::PRESSURE;
    }
    else if (free_memory > watermark_hi ){
...
ssize_t reclaimer::bytes_until_relaxed(pressure curr)
{
    if (curr >= pressure::NORMAL) {
        return watermark_hi - stats::free();
    } else {
        return 0;
    }
}
...
//     } 
//     else if (_oom_blocked.has_waiters()) {
//         // We wait if there is already waiter, instead of taking free pages 
//         // directly from the free_page_ranges
//         wait_for_memory(page_batch::nr_pages * mmu::page_size);
//     }
// }

void reclaimer::sync_reclaim_for_minimum_memory(){
    if (emergency_alloc_level){
        return;
    }
    if (stats::free() < min_emergency_pool_size){
        do_sync_reclaim(min_emergency_pool_size - stats::free());
    }
...
// If there is waiter, we wait
// void reclaimer::wait_for_memory_conditional(size_t mem)
// {
//     if (_oom_blocked._has_waiters()){
//         //Increase the atomic counter to force the reclaimer to reclaim more 
...
public:
    ...
    // Best effort to allocate n pages
    template<bool UseBitmap = true>
    unsigned alloc_n(size_t size, unsigned n, page_range* ptr_buf[], bool contiguous = true);
...
private:
    ...
    // For now we use mutex to protect each _free_huge and _free list 
};
...
{
    trace_pr_alloc(size,stats::free()); 
...
// Assume the lock is held outside
template<bool UseBitmap>
unsigned page_range_allocator::alloc_n(size_t size, unsigned n, page_range* ptr_buf[], bool contiguous){
    unsigned i = 0;
    while (i < n){
        page_range *pr = alloc<UseBitmap>(size, contiguous);
        if (pr){
            ptr_buf[i] = pr;
            i++;
        }
...
static void* malloc_large(size_t size, size_t alignment, bool block = true, bool contiguous = true)
{
    ...
    while (true) {
        reclaimer_thread.sync_reclaim_for_minimum_memory();
        WITH_LOCK(free_page_ranges_lock) {
            trace_memory_call_wait(11);
...
bool reclaimer_waiters::wake_waiters()
{
    ...
    free_page_ranges.for_each([&] (page_range& fp) {
        ...
        while (it != _waiters.end()) {
            auto& wr = *it;
            it++;
            trace_memory_wakeup_waiter(in_this_page_range, wr.bytes);
        ...
        trace_memory_loop(fp.size);
        
        return true;
    });
    
    // Here we try to see whether the wr.bytes request can be satisfied by 
    // Multiple regions. Wake as many waiters as possible. We use FIFO for 
    // now
    if (!woken){
        size_t total_free_page_ranges_size = stats::free(); 
        auto it = _waiters.begin();
        while (it != _waiters.end()) {
            auto& wr = *it;
            it ++;
            trace_memory_patch(total_free_page_ranges_size, wr.bytes); 
            if (total_free_page_ranges_size >= wr.bytes) {
                total_free_page_ranges_size -= wr.bytes;
                _waiters.erase(_waiters.iterator_to(wr));
                _waiters_count --;
                wr.owner->wake();
                wr.owner = nullptr;
...
reclaimer::reclaimer()
    : _oom_blocked()
{
    osv_reclaimer_thread = nullptr;
    for (unsigned i = 0; i < opt_max_reclaim_threads; i++){
        _reclaim_threads.push_back(std::unique_ptr<sched::thread>(
            sched::thread::make([&, i] {_do_concurrent_reclaim(i);}, 
            sched::thread::attr().name(osv::sprintf("concurrent_%u", i))
            .stack(mmu::page_size * 4))
        ));        
        _reclaim_threads[i]->start();
    }
}

// NOTE: This is a temporary hack for post init: (parse_cmdline) (loader)
// executed after the initialization 
void reclaimer::add_threads(size_t n_more_threads){
    size_t cur_id = _reclaim_threads.size();
    for (unsigned i = cur_id; i < cur_id + n_more_threads; i++){
        _reclaim_threads.push_back(std::unique_ptr<sched::thread>(
            sched::thread::make([&, i] {_do_concurrent_reclaim(i);}, 
            sched::thread::attr().name(osv::sprintf("concurrent_%u", i))
            .stack(mmu::page_size * 4))
...
// In concurrent reclaim, no wake up, no jvm, resemble the kswapd
void reclaimer::_do_concurrent_reclaim(size_t id)
{
    auto pinned_cpu = sched::cpus.size() > id + 1 ? sched::cpus.size() - id - 1: 0;
    sched::thread::current()->pin(sched::cpus[pinned_cpu]);
    ssize_t target = 0;
    ssize_t freed = 0;
    
    // Wait until the shrinker is added
    unsigned int local_active = 0;
    while(!local_active){
        WITH_LOCK(_shrinkers_mutex){
            local_active = _active_shrinkers;
        }
    }
    while (true){
        // The logic of adaptive asynchronization can be added here
        // The number of thread to wake up depends on the watermark

        // Wait until the condition is true or the conditional 
        // variable is woken
        ...
        while (target > freed || p > pressure::NORMAL)
        {
            for (auto s: _shrinkers){
                freed += s ->request_memory(target, false);
            }
            p = pressure_level();
        }
        trace_concurrent_working(target, freed, (size_t)p);
        freed = target = 0;
    }
    // TODO: Add JVM bollooning
    // TODO: cleanup the queue for application threads
}

// This function is called in synchronous reclaimation.
size_t reclaimer::do_sync_reclaim(size_t target){
    // Do we need to take reclaimer_lock here ??
    size_t freed = 0;
    for (auto s: _shrinkers){
        freed +=s -> request_memory(target, true);
    }
    trace_l2_sync_reclaim_working(target, freed);
    return freed;
    // TODO: Add JVM bollooning 
}
...
void free_page_n(unsigned n_pages, void* v_arr[], const std::string target);
...
STAT_ELAPSED(l1_refill);

// This small percpu buffer is only used asynchronous reclaim
struct reclaim_buf{
    reclaim_buf() : _nr(0) {}
    size_t _nr;
    void* _pages[page_batch::nr_pages];
    void* pop()
    {
        assert(_nr);
        return _pages[--_nr];
    ...
    void push(void* page)
    {
        assert(_nr < page_batch::nr_pages);
        _pages[_nr++] = page;
        if (_nr == page_batch::nr_pages){
            const std::string target = opt_no_global_l2 ? "range" : "l2";
            free_page_n(_nr, _pages, target);
            _nr = 0;
        }
    }
    void* top() { return _pages[_nr - 1]; }
};
...
struct l1 {
    ...
    {
        bool is_from_fill_thread = false;
        void* ret;
        while (!(ret = alloc_page_local())) {
            refill(is_from_fill_thread); // Not coming from fill_thread
        }
        return ret;
    }
    ...
    static void refill(bool is_from_fill_thread);
    static void unfill(bool is_from_fill_thread);
...
PERCPU(l1*, percpu_l1);
PERCPU(reclaim_buf*, percpu_reclaim_buf);
static sched::cpu::notifier _notifier([] () {
    ...
    *percpu_reclaim_buf = new reclaim_buf();
});
static inline l1& get_l1()
{
    return **percpu_l1;
}
static inline reclaim_buf& get_reclaim_buf()
{
    return **percpu_reclaim_buf;
}
...
public:
    ...
    // This function if only called via l1::refill()
    // So we can terminate it early in refill by calling the direct reclamation 
    // only once.
    page_batch* alloc_page_batch(bool from_fill_thread)
    {
        page_batch* pb = nullptr;
        trace_l2_alloc_page_batch(get_nr(), _watermark_lo, _watermark_hi);
        //uint64_t start_tick = processor::ticks();
        //uint64_t total_tick = 0;
        //uint16_t counter = 0;
        while (!pb) {
            pb = try_alloc_page_batch();
            if (pb) break;
            WITH_LOCK(migration_lock) {
                DROP_LOCK(preempt_lock) {
                    refill(from_fill_thread);
                }
                auto& pbuf = get_l1();
                // Break as long as L1 has pages so next l1::refill() should succeed
                if (pbuf.nr > 0){
                    break;
                }
            }
            //counter ++;
            //if (!pb) total_tick = processor::ticks() - start_tick;
        }

        //trace_l2_refill_all(processor::ticks() - start_tick);
        //if (total_tick){
        //    trace_l2_refill_wait(total_tick);
        //}
        return pb;
    }
    ...
    {
        trace_l2_free_page_batch(get_nr(), _watermark_lo, _watermark_hi);
    ...
    {
        // Instead of allocating directly, we need to wait if some other threads 
        // is also waiting. Otherwise we will starve the waiting threads
        page_batch* pb;
        if (!_queue.pop(pb)) {
            return nullptr;
        }
    ...
    void fill_thread();
    void refill(bool is_from_fill_thread);
    void unfill();
...
private:
    ...
    boost::lockfree::queue<page_batch*, boost::lockfree::fixed_sized<true>> _queue;
    std::unique_ptr<sched::thread> _fill_thread;
};
...
void l1::refill(bool is_from_fill_thread)
{
    STAT_ELAPSED_FROM_WRAP(l1_refill);
    SCOPE_LOCK(preempt_lock);
    auto& pbuf = get_l1();
    if (pbuf.nr == 0 && pbuf.nr + page_batch::nr_pages <= pbuf.max){
        if (opt_no_global_l2){
            // Try to allocate a batch directly from free_page_range
            page_batch batch;
            int n_alloc = 0;
            WITH_LOCK(free_page_ranges_lock) {
                n_alloc = free_page_ranges.alloc_n(
                    page_size, page_batch::nr_pages, 
                    reinterpret_cast<memory::page_range**>(batch.pages));
            }
            if (n_alloc){
                // TODO: Is there another thread which fills up pbuf?
                assert(pbuf.nr + page_batch::nr_pages <= pbuf.max);
                for (int i = 0; i < n_alloc; i++){
                    assert(batch.pages[i] != nullptr);
                    pbuf.push(batch.pages[i]);
                }
                on_alloc(n_alloc * page_size);
            } else if (opt_sync_reclaim == "no") {
                // Wait for async reclaim, loop on range allocator
                // Check page.hh for detailed workflow
                // TODO: Reexamine perf
                ...
                // Trigger sync reclaim if configured
                // TODO: Differentiate response from normal
                size_t n_sync_reclaim = 1;
                DROP_LOCK(preempt_lock){
                    reclaimer_thread.do_sync_reclaim(
                        n_sync_reclaim * page_size
                    );
                }
            }
        } else {
            auto* pb = global_l2.alloc_page_batch(is_from_fill_thread);
            if (pb) {
    ...
    STAT_ELAPSED_TO_WRAP(l1_refill);
}

void l1::unfill(bool is_from_fill_thread)
{
    SCOPE_LOCK(preempt_lock);
    auto& pbuf = get_l1();
    ...
    if (pbuf.nr > page_batch::nr_pages + pbuf.max / 2) {
        ...
        for (size_t i = 0 ; i < page_batch::nr_pages; i++) {
            pb->pages[i] = pbuf.pop();
            assert(pb->pages[i] != nullptr);
        }
        if (opt_no_global_l2){
            page_batch local_pb = *pb;
            WITH_LOCK(free_page_ranges_lock) {
                for (size_t i = 0 ; i < page_batch::nr_pages; i++) {
                    auto v = local_pb.pages[i];
                    assert(v != nullptr);
...
void* l1::alloc_page_local()
{
    ...
    trace_l1_alloc_page_local(pbuf.top(), pbuf.nr, pbuf.max);
    return pbuf.pop();
}
...
bool l1::free_page_local(void* v)
{
    ...
    trace_l1_free_page_local(v, pbuf.nr, pbuf.max);
    pbuf.push(v);
    return true;
}
...
STAT_ELAPSED(do_sync_reclaim_small);
STAT_ELAPSED(free_page_ranges_alloc);
STAT_ELAPSED(free_page_ranges_alloc_inside_lock);

void l2::refill(bool is_from_fill_thread)
{
    // Try to refill a batch from range allocator
    assert(!opt_no_global_l2);
    STAT_ELAPSED_FROM_WRAP(l2_refill);
    unsigned n_alloc = 0;
    page_batch batch;
    page_batch* pb;
    trace_l2_refill();
    pressure p = reclaimer::pressure_level();
    if (p >= pressure::PRESSURE){
        // Wake all under memory pressure
        trace_l2_concurrent_wake_all();
        reclaimer_thread.concurrent_wake_all();
    }
    // NOTE: Revisit
    //if (free_page_ranges.empty())
    //    return;
    // uint64_t start_cycles = processor::ticks();
    // uint64_t end_cycles = start_cycles;

    // TODO: Embed TLB Flush cooperative and check here
    if (opt_tlb_flush_mode == "patr"){
        // For patr, check cooperative flush 
        SCOPE_LOCK(migration_lock);
        SCOPE_LOCK(preempt_lock);
        auto cpu_id = sched::cpu::current() -> id;
        while (!free_page_ranges_lock.trylock()){
            mmu::flush_tlb_check_and_flush(cpu_id);
        }
        n_alloc = free_page_ranges.alloc_n(
            page_size, page_batch::nr_pages, 
            reinterpret_cast<memory::page_range**>(batch.pages)
        );
        free_page_ranges_lock.unlock();
    } else {
        WITH_LOCK(free_page_ranges_lock){
            // end_cycles = processor::ticks();
            n_alloc = free_page_ranges.alloc_n(
                page_size, page_batch::nr_pages, 
                reinterpret_cast<memory::page_range**>(batch.pages)
            );
    ...
    // trace_l2_refill_locking(end_cycles - start_cycles);

    if (n_alloc == page_batch::nr_pages){
        // Got a full batch
        on_alloc(page_batch::nr_pages * page_size);
        ...
        *pb = batch;
        ...
        // Got a partial batch, put as many as possible to L1
        assert(!sched::thread::current()->migratable());
        auto &pbuf = get_l1();
        // We charge as much as we can to l1 because free is very expensive
        size_t pushed = 0;
        while (pushed < n_alloc && pbuf.nr < pbuf.max){
            pbuf.push(batch.pages[pushed]);
            pushed ++;
        }
        if (pushed)
            on_alloc(pushed * page_size);

        // Free the rest of pages, should happen very infrequently
        if (__builtin_expect((pushed < n_alloc), 0)) {
            WITH_LOCK(free_page_ranges_lock){
                for (; pushed < n_alloc; pushed++){
                    auto v = batch.pages[pushed];
                    assert(v != nullptr);
                    auto pr = new (v) page_range(page_size);
        ...
        // Got 0 page, sync reclaim for one batch pages
        // TODO: Reexamine number of pages
        STAT_ELAPSED_FROM_WRAP(do_sync_reclaim_small);
        size_t n_sync_reclaim_page = page_batch::nr_pages;
        reclaimer_thread.do_sync_reclaim(
            n_sync_reclaim_page * page_size);
        STAT_ELAPSED_TO_WRAP(do_sync_reclaim_small);
    } else {
        trace_l2_refill_wait_for_page();
    }
    // else no sync reclaim, will loop on alloc_page_batch until succeed
    if (n_alloc == 0)
        trace_l2_refill_ret();
    STAT_ELAPSED_TO_WRAP(l2_refill);
}
...
STAT_ELAPSED(l2_free_batch);
STAT_ELAPSED(l2_free_batch_inside_lock);
void l2::free_batch(page_batch& batch)
{
    STAT_ELAPSED_FROM_WRAP(l2_free_batch);
    WITH_LOCK(free_page_ranges_lock) {
        STAT_ELAPSED_FROM_WRAP(l2_free_batch_inside_lock);
        for (size_t i = 0 ; i < page_batch::nr_pages; i++) {
        ...
        STAT_ELAPSED_TO_WRAP(l2_free_batch_inside_lock);
    }
    STAT_ELAPSED_TO_WRAP(l2_free_batch);
}
...
static void early_free_page_n(int n_pages, void* v_arr[]){
    // TODO: Reexamine the performance of new here 
    WITH_LOCK(free_page_ranges_lock) {
        for (int i = 0; i < n_pages; i++){
            auto pr = new (v_arr[i]) page_range(page_size);
            free_page_range_locked(pr);
        }
    }
}
...
void free_page_to_reclaim_buf(void* v){
    auto& pbuf = page_pool::get_reclaim_buf();
    pbuf.push(v);
}

void free_page_to_range(void* v){
    early_free_page(v);
    tracker_forget(v);
}

void free_page_n(unsigned n_pages, void* v_arr[], const std::string target){
    if (!smp_allocator || target == "range" || 
        (target == "l2" && n_pages < page_batch::nr_pages)) {
        early_free_page_n(n_pages, v_arr);
        tracker_forget_n(n_pages, v_arr);
    } else if (target == "l1") {
        for (unsigned i = 0; i < n_pages; i++){
            page_pool::l1::free_page(v_arr[i]);
        }
        tracker_forget_n(n_pages, v_arr);
    } else if (target == "l2") {
        assert(!opt_no_global_l2);
        // Form batched and then put the rest to range
        unsigned idx = 0;
        while (idx + page_batch::nr_pages <= n_pages){
            auto* pb = static_cast<page_batch*>(v_arr[idx]);
            for (size_t i = idx ; i < idx + page_batch::nr_pages; i++) {
                pb->pages[i] = v_arr[i];
            }
            page_pool::global_l2.free_page_batch(pb);
            idx += page_batch::nr_pages;
        }
        if (idx < n_pages){
            early_free_page_n(n_pages - idx, v_arr + idx);
        }
...
void clear_reclaim_buf(){
    ...
    if (pbuf._nr){
        const std::string target = opt_no_global_l2 ? "range" : "l2";
        free_page_n(pbuf._nr, pbuf._pages, target);
        pbuf._nr = 0;
    }
}
...
void* malloc(size_t size, size_t alignment)
{
    ...
    //     trace_memory_call_wait(3);
    //     memory::reclaimer_thread.wait_for_minimum_memory();
    // }
    memory::reclaimer_thread.sync_reclaim_for_minimum_memory();
...
void add_additional_reclaim_threads(size_t n_more_threads){
    reclaimer_thread.add_threads(n_more_threads);
}
...
void reduce_total_memory(size_t memory){
    ...
    memory::watermark_hi = memory::stats::total() * 90 / 100;
}
}