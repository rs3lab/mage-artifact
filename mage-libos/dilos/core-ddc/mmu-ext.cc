...
STAT_ELAPSED(ddc_page_fault_remote_addr_lock);
STAT_ELAPSED(ddc_page_fault_remote_pte);
STAT_ELAPSED(ddc_page_fault_remote_alloc);
STAT_ELAPSED(ddc_page_fault_remote_fetch);
STAT_ELAPSED(ddc_page_fault_remote_poll);
STAT_ELAPSED(ddc_page_fault_prefetching);
STAT_ELAPSED(ddc_page_fault_dispatching);
STAT_ELAPSED(ddc_page_fault_waiting);
STAT_ELAPSED(ddc_minor_fault_memset);

PERCPU(size_t, maj_flts);
PERCPU(size_t, min_flts);
PERCPU(uint64_t, _percpu_rdma_cycles);
PERCPU(uint64_t, _percpu_rdma_sched_cycles);
...
TRACEPOINT(trace_ddc_mmu_handle_insert_done,"");
...
static void handle_init(unsigned my_cpu_id, uintptr_t va) {
    ...
    trace_ddc_mmu_handle_insert_done();
}
...
static int do_polling(unsigned my_cpu_id, int &remain) {
    ...
    auto start_poll = processor::ticks();
    int polled = ctx.queues.poll(tokens, to_poll);
    auto end_poll = processor::ticks();
    (*_percpu_rdma_cycles) += end_poll - start_poll;
    trace_ddc_mmu_polled_size(polled);
    STAT_ELAPSED_FROM_WRAP(ddc_page_fault_remote_pte);
    ...
    STAT_ELAPSED_TO_WRAP(ddc_page_fault_remote_pte);
    try_flush_buffered();
    return polled;
}
class first_fault_handling {
   protected:
    bool do_page_small_first(mmu::pt_element<0> pte, mmu::hw_ptep<0> ptep,
        ...
        assert(paddr != NULL);
        STAT_ELAPSED_FROM_WRAP(ddc_minor_fault_memset);
        memset(paddr, 0, mmu::page_size);  // TODO: no zerofill
        STAT_ELAPSED_TO_WRAP(ddc_minor_fault_memset);
        ...
        WITH_LOCK(preempt_lock) { 
            n_faultin.fetch_add(1, std::memory_order::memory_order_relaxed);
            insert_page_buffered(page); 
        }
        return true;
    }
};
...
class fault_handling
   ...
   private:
    bool do_page_small_remote(mmu::pt_element<0> pte, mmu::hw_ptep<0> ptep,
        ...
        STAT_ELAPSED_TO(ddc_page_fault_dispatching, __debug);
        STAT_ELAPSED_FROM_WRAP(ddc_page_fault_remote_addr_lock);
        while (true) {
            if (pte.valid()) {
        ...
        STAT_ELAPSED_TO_WRAP(ddc_page_fault_remote_addr_lock);

        // 2. alloc page to fetch
        STAT_ELAPSED_FROM_WRAP(ddc_page_fault_remote_alloc);
        void *paddr = ::memory::alloc_page();
        STAT_ELAPSED_TO_WRAP(ddc_page_fault_remote_alloc);
        assert(paddr != NULL);
        trace_ddc_mmu_fault_remote_offset(va);
        // uintptr_t fault_page_addr = align_down(_fault_addr,
        // mmu::page_size);

        STAT_ELAPSED_FROM_WRAP(ddc_page_fault_remote_fetch);
        auto start_fetch = processor::ticks();
        WITH_LOCK(preempt_lock) {
            auto &ctx = percpu_ctx[my_cpu_id];
            ...
            ctx.issued += 1;
            auto end_fetch = processor::ticks();
            (*_percpu_rdma_cycles) += end_fetch - start_fetch;
            STAT_ELAPSED_TO_WRAP(ddc_page_fault_remote_fetch);
            STAT_ELAPSED_FROM_WRAP(ddc_page_fault_prefetching);
            ...
            int handler_ret = ev_handler(&ev.inner);
            assert(handler_ret == 0);
            STAT_ELAPSED_TO_WRAP(ddc_page_fault_prefetching);
        }

        STAT_ELAPSED_FROM_WRAP(ddc_page_fault_remote_poll);
        WITH_LOCK(preempt_lock) {
            n_faultin.fetch_add(remain, std::memory_order::memory_order_relaxed);
            while (remain > 0) {
                if (opt_tlb_flush_mode == "patr")
                    flush_tlb_check_and_flush(my_cpu_id); 
                STAT_ELAPSED_FROM_WRAP(ddc_page_fault_waiting);
                auto start_poll = processor::ticks();
                // DROP_LOCK(preempt_lock) { sched::cpu::schedule(); }
                auto end_poll = processor::ticks();
                (*_percpu_rdma_cycles) += end_poll - start_poll;
                (*_percpu_rdma_sched_cycles) += end_poll - start_poll;
                STAT_ELAPSED_TO_WRAP(ddc_page_fault_waiting);
        ...
        STAT_ELAPSED_TO_WRAP(ddc_page_fault_remote_poll);
...
static std::array<remote_queue, max_cpu> percpu_syscall_queue;
...
class pageout : public mmu::vma_operation<mmu::allocate_intermediate_opt::no,
   ...
   public:
    ...
    pageout()
        : cleaner_madv(percpu_syscall_queue, 0, max_syscall),
          tlb_pushed(ddc::tlb_flusher_madv::max_tokens),
          pushed(max_syscall),
          to_free(max_free) {
        spinlock_init(&tlb_pushed_lock);
        spinlock_init(&pushed_lock);
        spinlock_init(&to_free_lock);
    }
    
    pageout(const pageout&) = delete;
        ...
        while (true) {
            ...
            switch (ret) {
                ...
                case state::OK_TLB_PUSHED:
                    tlb_pushed_lock.lock();
                    tlb_pushed.push_back({ptep, va});
                    tlb_pushed_lock.unlock();
                    return true;
    ...
    void do_free() {
        // This is incorrect because read is not protected by lock
        // The performance can be very poor because of 
        // frequent lock and unlock
        // just make it compile
        for (mmu::phys paddr : to_free) {
            page_list.free_paddr(paddr);
        }
        to_free.clear();
    }

    void try_free(mmu::phys paddr) {
        SCOPE_LOCK(to_free_lock);
   ...
   private:
    ...
    spinlock_t tlb_pushed_lock;
    spinlock_t pushed_lock;
    spinlock_t to_free_lock;
};
...
// Don't use the same class as the tlb_flusher which is used by evictor
class madv_freeing
    : public mmu::vma_operation<mmu::allocate_intermediate_opt::no,
                                mmu::skip_empty_opt::yes,
                                mmu::account_opt::yes>,
      public tlb_flusher_madv {
   public:
    madv_freeing() : tlb_flusher_madv() {}
    ~madv_freeing() {
        // Cannot put the tlb_flush function in the destructor of the 
        // base class (tlb_flusher_madv) because it will call the pure 
        // virtual function in the base class not the one here we have 
        // overriden:
        tlb_flush();
    }
    ...
    bool page(mmu::hw_ptep<0> ptep, uintptr_t va) {
        while (true) {
            ...
            if (pte.accessed()) {
                ...
                SCOPE_LOCK(migration_lock);
                // The reclamation thread can access it concurrently
    ...
    // Do nothing because the page are still in the active list
    // They are not owned by the tlb flush queue
    void tlb_flush_after(uintptr_t token) override {
        return;
    }
};
...
class madv_dontneed
    : public mmu::vma_operation<mmu::allocate_intermediate_opt::no,
      ...
      public tlb_flusher {
   public:
    madv_dontneed() : tlb_flusher() {spinlock_init(&to_free_lock);}
    ...
    void tlb_flush_after(uintptr_t token) override {
        ...
        if (page.st == page_status::TRACK_ACTIVE ||
            ...
            // This can be incorrect. Read is not protected by the lock
            page_list.pop_page(page);
...
int madvise_pageout(void *addr, size_t length) {
    ...
    // Disable migration for pageout because we use per-core 
    // remote queue
    SCOPE_LOCK(migration_lock);

    // TODO: This function is not correct anyway.
    // must be revisited if used by any.
    pageout pto;
    ddc_operate_range(pto, addr, length);
...
int madvise_cooperative_tlb_with_check(unsigned current_cpu){
    SCOPE_LOCK(preempt_lock);
    unsigned my_cpu_id = sched::cpu::current()->id;
    if (my_cpu_id == current_cpu && opt_tlb_flush_mode == "patr"){
        // Not migrated
        flush_tlb_check_and_flush(my_cpu_id); 
    }
    return 0;
...
int madvise_print_stat() {
    ...
    for (size_t i = 0; i < sched::cpus.size(); ++i) {
        ...
        percpu_syscall_queue[i].get_stat(fetched, pushed);
#ifdef PRINT_ALL
        debug_early_u64("Syscall: ", i);
        debug_early_u64("  fetched: ", fetched);
        debug_early_u64("  pushed : ", pushed);
#endif
        fetched_total += fetched;
...
int madvise(void *addr, size_t length, int advice) {
    ...
    switch (advice2) {
        ...
        case MADV_DDC_COOP_TLB:
            return madvise_cooperative_tlb();
        default:
...
class mlocking
   ...
   public:
    ...
    bool page_inner(mmu::pt_element<0> pte) {
        // This mlock is incorrect. There can be a race condition
        mmu::phys paddr = pte.addr();

        auto &page = ipt.lookup(paddr);
        int list_id = page.list_id;
        if (list_id != -1){
            active_list_locks[list_id].lock();
            clean_list_locks[list_id].lock();
        }
        ...
        if (list_id != -1){
            clean_list_locks[list_id].unlock();
            active_list_locks[list_id].unlock();
        }
...
void ddc_init() {
    ...
    for (size_t i = 0; i < sched::cpus.size(); ++i) {
        ...
        percpu_syscall_queue[i].add_push(max_syscall);
        percpu_syscall_queue[i].setup();
    }
