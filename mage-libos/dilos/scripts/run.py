...
def start_osv_qemu(options):
    ...
    if options.arch == 'aarch64':
        ...
        args = [
            "-m", options.memsize,
#            "-m", "60G",
#            "-smp", "cores=28,threads=1,sockets=1", 
#            "-numa", "node,nodeid=0,memdev=nr0,cpus=0-27",
#            "-numa", "node,nodeid=1,memdev=nr1,cpus=28-55",
#            "-object", f"memory-backend-ram,size={options.memsize},id=nr0"]
#            "-object", f"memory-backend-ram,size={options.memsize},id=nr1"]
            "-smp", options.vcpus]
#            "-object", "memory-backend-file,id=mem,size=4M,mem-path=/dev/shm,share=on"]
    ...
    for idx in range(int(options.nics)):
        ...
        if options.networking:
            ...
            net_device_options.extend(['mq=on', 'packed=on','netdev=hn%d' % idx, 'id=nic%d' % idx])
        else:
            if options.api:
    ...
    if options.hypervisor == "kvm" or options.hypervisor == 'qemu_microvm':
        args += ["-enable-kvm", "-cpu", "host,+x2apic,+avx,+avx2"]
