...
B<qemu-system-x86_64> [I<options>] [I<disk_image>]
...
=item B<-smp [cpus=]>I<n>B<[,cores=>I<cores>B<][,threads=>I<threads>B<][,dies=dies][,sockets=>I<sockets>B<][,maxcpus=>I<maxcpus>B<]>

Simulate an SMP system with I<n> CPUs. On the PC target, up to 255
CPUs are supported. On Sparc32 target, Linux limits the number of usable CPUs
to 4.
For the PC target, the number of I<cores> per die, the number of I<threads>
per cores, the number of I<dies> per packages and the total number of
I<sockets> can be specified. Missing values will be computed.
If any on the three values is given, the total number of CPUs I<n> can be omitted.
I<maxcpus> specifies the maximum number of hotpluggable CPUs.
...
=item B<-audiodev [driver=]>I<driver>B<,id=>I<id>B<[,>I<prop>B<[=>I<value>B<][,...]]>

Adds a new audio backend I<driver> identified by I<id>.  There are
global and driver specific properties.  Some values can be set
differently for input and output, they're marked with C<in|out.>.
You can set the input's property with C<in.I<prop>> and the
output's property with C<out.I<prop>>. For example:
...
=item B<id=>I<identifier>

Identifies the audio backend.


=item B<timer-period=>I<period>

Sets the timer I<period> used by the audio subsystem in microseconds.
Default is 10000 (10 ms).


=item B<in|out.mixing-engine=on|off>
...
I<fixed-settings> must be off too.  Note that disabling this option
...
=item B<in|out.fixed-settings=on|off>

Use fixed settings for host audio.  When off, it will change based on
how the guest opens the sound card.  In this case you must not specify
I<frequency>, I<channels> or I<format>.  Default is on.


=item B<in|out.frequency=>I<frequency>

Specify the I<frequency> to use when using I<fixed-settings>.
Default is 44100Hz.


=item B<in|out.channels=>I<channels>

Specify the number of I<channels> to use when using
I<fixed-settings>. Default is 2 (stereo).


=item B<in|out.format=>I<format>

Specify the sample I<format> to use when using I<fixed-settings>.
Valid values are: C<s8>, C<s16>, C<s32>, C<u8>,
C<u16>, C<u32>. Default is C<s16>.


=item B<in|out.voices=>I<voices>

Specify the number of I<voices> to use.  Default is 1.


=item B<in|out.buffer-length=>I<usecs>
...
=item B<-audiodev none,id=>I<id>B<[,>I<prop>B<[=>I<value>B<][,...]]>

Creates a dummy backend that discards all outputs.  This backend has no
backend specific properties.


=item B<-audiodev alsa,id=>I<id>B<[,>I<prop>B<[=>I<value>B<][,...]]>
...
=item B<in|out.dev=>I<device>

Specify the ALSA I<device> to use for input and/or output.  Default
is C<default>.


=item B<in|out.period-length=>I<usecs>

Sets the period length in microseconds.


=item B<in|out.try-poll=on|off>
...
=item B<-audiodev coreaudio,id=>I<id>B<[,>I<prop>B<[=>I<value>B<][,...]]>
...
=item B<in|out.buffer-count=>I<count>

Sets the I<count> of the buffers.
...
=item B<-audiodev dsound,id=>I<id>B<[,>I<prop>B<[=>I<value>B<][,...]]>
...
=item B<latency=>I<usecs>

Add extra I<usecs> microseconds latency to playback.  Default is
10000 (10 ms).
...
=item B<-audiodev oss,id=>I<id>B<[,>I<prop>B<[=>I<value>B<][,...]]>
...
Specify the file name of the OSS I<device> to use.  Default is
C</dev/dsp>.
...
=item B<in|out.try-poll=on|of>

Attempt to use poll mode with the device.  Default is on.


=item B<try-mmap=on|off>

Try using memory mapped device access.  Default is off.


=item B<exclusive=on|off>

Open the device in exclusive mode (vmix won't work in this case).
Default is off.


=item B<dsp-policy=>I<policy>
...
specified by C<buffer> and C<buffer-count>.  This option is
...
=item B<-audiodev pa,id=>I<id>B<[,>I<prop>B<[=>I<value>B<][,...]]>
...
Sets the PulseAudio I<server> to connect to.


=item B<in|out.name=>I<sink>

Use the specified source/sink for recording/playback.


=item B<in|out.latency=>I<usecs>
...
=item B<-audiodev sdl,id=>I<id>B<[,>I<prop>B<[=>I<value>B<][,...]]>
...
=item B<-audiodev spice,id=>I<id>B<[,>I<prop>B<[=>I<value>B<][,...]]>

Creates a backend that sends audio through SPICE.  This backend requires
C<-spice> and automatically selected in that case, so usually you
can ignore this option.  This backend has no backend specific
properties.


=item B<-audiodev wav,id=>I<id>B<[,>I<prop>B<[=>I<value>B<][,...]]>
...
=item B<-device ipmi-bmc-sim,id=>I<id>B<[,slave_addr=>I<val>B<][,sdrfile=>I<file>B<][,furareasize=>I<val>B<][,furdatafile=>I<file>B<][,guid=>I<uuid>B<]>
...
B<read-only=off> does not work and the option must be specified
explicitly.

=item C<auto-read-only>

If B<auto-read-only=on> is set, QEMU may fall back to read-only usage
even when B<read-only=off> is requested, or even switch between modes as
...
Enabling B<force-share=on> requires B<read-only=on>.
...
=item B<-fsdev local,id=>I<id>B<,path=>I<path>B<,security_model=>I<security_model> B<[,writeout=>I<writeout>B<][,readonly][,fmode=>I<fmode>B<][,dmode=>I<dmode>B<] [,throttling.>I<option>B<=>I<value>B<[,throttling.>I<option>B<=>I<value>B<[,...]]]>


=item B<-fsdev proxy,id=>I<id>B<,socket=>I<socket>B<[,writeout=>I<writeout>B<][,readonly]>


=item B<-fsdev proxy,id=>I<id>B<,sock_fd=>I<sock_fd>B<[,writeout=>I<writeout>B<][,readonly]>


=item B<-fsdev synth,id=>I<id>B<[,readonly]>
...
=item B<throttling.bps-total=>I<b>B<,throttling.bps-read=>I<r>B<,throttling.bps-write=>I<w>

Specify bandwidth throttling limits in bytes per second, either for all request
types or for reads or writes only.

=item B<throttling.bps-total-max=>I<bm>B<,bps-read-max=>I<rm>B<,bps-write-max=>I<wm>

Specify bursts in bytes per second, either for all request types or for reads
or writes only.  Bursts allow the guest I/O to spike above the limit
temporarily.

=item B<throttling.iops-total=>I<i>B<,throttling.iops-read=>I<r>B<, throttling.iops-write=>I<w>

Specify request rate limits in requests per second, either for all request
types or for reads or writes only.

=item B<throttling.iops-total-max=>I<im>B<,throttling.iops-read-max=>I<irm>B<, throttling.iops-write-max=>I<iwm>

Specify bursts in requests per second, either for all request types or for reads
or writes only.  Bursts allow the guest I/O to spike above the limit temporarily.

=item B<throttling.iops-size=>I<is>
...
=item B<-device virtio-9p->I<type>B<,fsdev=>I<id>B<,mount_tag=>I<mount_tag>
...
=item B<-virtfs local,path=>I<path>B<,mount_tag=>I<mount_tag> B<,security_model=>I<security_model>B<[,writeout=>I<writeout>B<][,readonly] [,fmode=>I<fmode>B<][,dmode=>I<dmode>B<][,multidevs=>I<multidevs>B<]>


=item B<-virtfs proxy,socket=>I<socket>B<,mount_tag=>I<mount_tag> B<[,writeout=>I<writeout>B<][,readonly]>


=item B<-virtfs proxy,sock_fd=>I<sock_fd>B<,mount_tag=>I<mount_tag> B<[,writeout=>I<writeout>B<][,readonly]>


=item B<-virtfs synth,mount_tag=>I<mount_tag>
...
Please use C<-fsdev synth> and C<-device virtio-9p-...> instead.
...
support USB 3.0.  In this case B<-device qemu-xhci> can be used instead
...
C<charset> option, for example C<charset=CP850> for IBM CP850
encoding. The default is C<CP437>.
...
of two C<authz-list> objects with IDs of C<vnc.username> and
C<vnc.x509dname>. The rules for these objects must be configured
with the HMP ACL commands.

This option is deprecated and should no longer be used. The new
B<sasl-authz> and B<tls-authz> options are a
replacement.
...
=item B<audiodev=>I<audiodev>


Use the specified I<audiodev> when the VNC client requests audio
...
=item B<-smbios type=2[,manufacturer=>I<str>B<][,product=>I<str>B<][,version=>I<str>B<][,serial=>I<str>B<][,asset=>I<str>B<][,location=>I<str>B<]>
...
=item B<-chardev socket,id=>I<id>B<[,>I<TCP options> B<or> I<unix options>B<][,server][,nowait][,telnet][,websocket][,reconnect=>I<seconds>B<][,tls-creds=>I<id>B<][,tls-authz=>I<id>B<]>
...
B<tls-auth> provides the ID of the QAuthZ authorization object against
...
with I<number>.  This does not affect crypto routines within the host.
...
=item B<-semihosting-config [enable=on|off][,target=native|gdb|auto][,chardev=id][,arg=str[,...]]>
...
=item B<-plugin file=>I<file>B<[,arg=>I<string>B<]>
...
file metadata is in sync for B<mem-path> in case of host crash
or a power failure. MAP_SYNC requires support from both the host kernel
(since Linux kernel 4.15) and the filesystem of B<mem-path> mounted
with DAX option.
...
=item B<-object rng-builtin,id=>I<id>


Creates a random number generator backend which obtains entropy from
QEMU builtin functions. The B<id> parameter is a unique ID that
will be used to reference this entropy backend from the B<virtio-rng>
device. By default, the B<virtio-rng> device uses this RNG backend.
...
entropy from and if omitted defaults to B</dev/urandom>.
...
=item B<-object colo-compare,id=>I<id>B<,primary_in=>I<chardevid>B<,secondary_in=>I<chardevid>B<,outdev=>I<chardevid>B<,iothread=>I<id>B<[,vnet_hdr_support][,notify_dev=>I<id>B<]>
...
# qemu-system-x86_64 -object secret,id=sec0,file=mypasswd.txt,format=raw
...
=item B<-object authz-simple,id=>I<id>B<,identity=>I<string>


Create an authorization object that will control access to network services.

The B<identity> parameter is identifies the user and its format
...
=item B<-object authz-listfile,id=>I<id>B<,filename=>I<path>B<,refresh=>I<yes|no>


Create an authorization object that will control access to network services.

The B<filename> parameter is the fully qualified path to a file
...
the first rule to match will have its B<policy> value returned
as the result. If no rules match, then the default B<policy>
value is returned.
...
If B<refresh> is set to true the file will be monitored
and automatically reloaded whenever its content changes.

As with the C<authz-simple> object, the format of the identity
...
=item B<-object authz-pam,id=>I<id>B<,service=>I<string>


Create an authorization object that will control access to network services.

The B<service> parameter provides the name of a PAM service to use
for authorization. It requires that a file C</etc/pam.d/I<service>>
exist to provide the configuration for the C<account> subsystem.
...
C</etc/pam.d/qemu-vnc> that contains:
...
Finally the C</etc/qemu/vnc.allow> file would contain
