...
static struct ibv_qp_ex *create_qp(struct ibv_cq *cq, int max_send_wr,
    ...
    // Manually signaling all entries
    qp_init_attr_ex.sq_sig_all = 0;
    //qp_init_attr_ex.sq_sig_all = opt_rdma_mode == "selective" ? 0 : 1;
...
bool remote_queue::push_multiple(int qp_id, int n, const std::array<uintptr_t, N>& tokens,
    const std::array<void*, N> &paddrs, const std::array<uintptr_t, N>& offsets, size_t size){
    assert(n < N);
    auto &qp = push_qp[qp_id];
    struct ibv_qp_ex *qpx = reinterpret_cast<ibv_qp_ex *>(qp.inner);
    assert(qpx!=NULL);
    ...
    for (int i = 0; i < n; i++){
        ...
        auto &meta = find_mr(paddrs[i], size);
        ibv_wr_rdma_write(qpx, remote_server.rkey, remote_server.start + offsets[i]);
        ibv_wr_set_sge(qpx, meta.lkey, (uint64_t)paddrs[i], size);
    }
    int ret = ibv_wr_complete(qpx);
    assert(ret == 0);
    return true;
...
template bool remote_queue::push_multiple<pipeline_batch_size>(int, int, 
    const std::array<uintptr_t, pipeline_batch_size>&, 
    const std::array<void*, pipeline_batch_size>&, 
    const std::array<uintptr_t, pipeline_batch_size>&, 
    size_t);
...
void socket_init() {
    ...
    fprintf(stdout, "%s %d\n", options::ms_ip.c_str(), options::ms_port);
    if (ret < 0) {
